        ...
        try:
            ...
            async with session.get(check_url, timeout=10) as resp:
                if (await resp.text()).strip() == "registered":
                    await message.answer("✅ Вы верифицированы!\nГруппа: https://t.me/RAI_service_group")
                    return 
        except Exception: pass

    WELCOME_VIDEO_ID = "BAACAgIAAxkBAAIEA2lb6OOI4SgnsGqibms4cbfJCQWzAAJvkQAC5bngSoh4P26VZWLsOAQ"
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Начать", callback_data="paid")]])
    try:
        await message.answer_video(video=WELCOME_VIDEO_ID, caption="👋 Пройдите анкету для доступа.", reply_markup=kb, parse_mode="HTML")
    except:
        await message.answer("👋 Пройдите анкету для доступа.", reply_markup=kb)
...
async def main():
    ...
    # Удаляем старые обновления, чтобы бот не «захлебнулся» при старте
    await bot.delete_webhook(drop_pending_updates=True)
    # Запускаем рассылку
    asyncio.create_task(check_and_send())
    logger.info("--- БОТ УСПЕШНО ЗАПУЩЕН ---")
    # Разрешаем все нужные типы обновлений
